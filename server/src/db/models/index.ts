...
const configFile = require(path.join(__dirname, '/../config/database.json'));
const config: DatabaseConfig = configFile[env];
...
if (config.use_env_variable) {
  ...
  sequelize = new Sequelize(envVariable);
} else {
...
Object.keys(db).forEach((modelName) => {
  const model = db[modelName] as { associate?: (models: DB) => void };
  if (model.associate) {
    model.associate(db);
  }
});
